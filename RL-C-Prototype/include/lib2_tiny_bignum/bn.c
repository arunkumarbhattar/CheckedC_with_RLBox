...
void bignum_to_string(struct bn* n : itype(_Ptr<struct bn>), char* str: itype(_Nt_array_ptr<char>), int nbytes)
{
  ...
  {
    _Nt_array_ptr<char> read_str : count(1) = _Dynamic_bounds_cast<_Nt_array_ptr<char>>(read_str, bounds(str + i, str + i + 1)); 
    snprintf(_Dynamic_bounds_cast<_Nt_array_ptr<char>>(read_str, bounds(str + i, str + i + 1)), nbytes - i + 1, SPRINTF_FORMAT_STR, n->array[j]);
