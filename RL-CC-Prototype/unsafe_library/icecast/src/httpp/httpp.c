...
/* misc */
static char *_lowercase(char *str : itype(_Ptr<char>)) : itype(_Ptr<char>);
...
static int split_headers(char *data : itype(_Ptr<char>), unsigned long len, char **line : itype(_Ptr<char *>))
{
...
static void parse_headers(_Ptr<http_parser_t> parser, char **line : itype(_Ptr<char *>), int lines)
{
    int i, l;
    int whitespace, slen;
    char *name = NULL;
...
static char *url_escape(const char *src : itype(_Ptr<const char>)) : itype(_Ptr<char>)
{
    int len = strlen(src);
...
static void parse_query(_Ptr<http_parser_t> parser, char *query : itype(_Ptr<char>))
{
    int len;
    int i=0;
    char *key = query;
    ...
    while(i<len) {
        ...
        case '&':
            query[i] = 0;
            if(val && key)
                httpp_set_query_param(parser, key, _Assume_bounds_cast<_Ptr<const char>>(val));
            key = query+i+1;
            break;
        case '=':
            query[i] = 0;
