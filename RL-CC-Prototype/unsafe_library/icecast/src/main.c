...
static void _fatal_error(const char *perr : itype(_Ptr<const char>))
{
#ifdef WIN32_SERVICE
...
static int _parse_config_opts(int argc, char **argv : itype(_Ptr<_Ptr<char>>), char *filename : itype(_Ptr<char>), int size)
{
    int i = 1;
    int config_ok = 0;
...
#endif
{
    ...
    */
    ...
    if (res == 1) {
        ...
        config_get_config();
        ret = config_initial_parse_file(_Assume_bounds_cast<_Ptr<const char>>(filename));
