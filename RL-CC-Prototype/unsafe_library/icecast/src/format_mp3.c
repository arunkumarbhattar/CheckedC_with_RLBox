...
static void format_mp3_free_plugin(_Ptr<format_plugin_t> self);
static _Ptr<refbuf_t> mp3_get_filter_meta(_Ptr<source_t> source);
static _Ptr<refbuf_t> mp3_get_no_meta(_Ptr<source_t> source);

static int  format_mp3_create_client_data (_Ptr<source_t> source, _Ptr<client_t> client);
static void free_mp3_client_data (_Ptr<client_t> client);
static int format_mp3_write_buf_to_client(_Ptr<client_t> client);
static void write_mp3_to_file (_Ptr<struct source_tag> source, _Ptr<refbuf_t> refbuf);
static void mp3_set_tag (_Ptr<format_plugin_t> plugin, const char *tag : itype(_Ptr<const char>), const char *in_value : itype(_Ptr<const char>), _Ptr<const char> charset);
static void format_mp3_apply_settings(_Ptr<client_t> client, _Ptr<format_plugin_t> format, _Ptr<mount_proxy> mount);
...
int format_mp3_get_plugin (_Ptr<struct source_tag> source)
{
    ...
    mp3_state *state = calloc<mp3_state>(1, sizeof(mp3_state));
    _Ptr<refbuf_t> meta = ((void *)0);
    ...
    meta = refbuf_new (17);
    memcpy<char> (meta->data, "\001StreamTitle='';", 17);
    state->metadata = meta;
    state->interval = -1;
...
static void mp3_set_tag (_Ptr<format_plugin_t> plugin, const char *tag : itype(_Ptr<const char>), const char *in_value : itype(_Ptr<const char>), _Ptr<const char> charset)
{
    ...
    {
        free<char> (source_mp3->url_title);
        source_mp3->url_title = value;
    }
    else if (strcmp (tag, "artist") == 0)
    {
        free<char> (source_mp3->url_artist);
        source_mp3->url_artist = value;
    }
    else if (strcmp (tag, "url") == 0)
    {
        free<char> (source_mp3->url);
...
static void filter_shoutcast_metadata (_Ptr<source_t> source, char *metadata : itype(_Ptr<char>), unsigned int meta_len)
{
    if (metadata)
    {
        char *end, *p;
        ...
        do
        {
            ...
            if ((end = strstr (metadata+13, "\';")) == NULL)
                break;
            len = (end - metadata) - 13;
            p = calloc<char> (1, len+1);
            if (p)
            {
                memcpy<char> (p, metadata+13, len);
                logging_playlist (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>(p), source->listeners);
                stats_event_conv (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("title"), p, source->format->charset);
 ...
 */
static void mp3_set_title (_Ptr<source_t> source)
{
    const char streamtitle[] = "StreamTitle='";
    const char streamurl[] = "StreamUrl='";
    size_t size;
    unsigned char len_byte;
    _Ptr<refbuf_t> p = ((void *)0);
 ...
 */
static int send_stream_metadata (_Ptr<client_t> client, _Ptr<refbuf_t> associated)
{
    int ret = 0;
    char *metadata;
    int meta_len;
...
static int format_mp3_write_buf_to_client(_Ptr<client_t> client)
{
    ...
    do
    {
        ...
        {
            _Ptr<refbuf_t> associated = refbuf->associated;
...
static void format_mp3_free_plugin(_Ptr<format_plugin_t> self)
{
    ...
    free<char> (state->url_artist);
    free<char> (state->url_title);
    free<char> (self->charset);
    refbuf_release (state->metadata);
    refbuf_release (state->read_data);
    free<mp3_state>(state);
    free<format_plugin_t>(self);
}
 ...
 */
static int complete_read (_Ptr<source_t> source)
{
    int bytes;
...
#define REFBUF_SIZE 1400
    ...
    bytes = client_read_bytes (_Assume_bounds_cast<_Ptr<client_t>>(source->client), buf, REFBUF_SIZE-source_mp3->read_count);
    if (bytes < 0)
    {
...
{
    _Ptr<refbuf_t> refbuf = ((void *)0);
    _Ptr<format_plugin_t> plugin = source->format;
    mp3_state *source_mp3 = plugin->_state;
    unsigned char *src;
    ...
    while (bytes > 0)
    {
                ...
                strcmp (source_mp3->build_metadata+1, source_mp3->metadata->data+1) != 0)
        {
            ...
            memcpy<char> (meta->data, source_mp3->build_metadata,
...
static int format_mp3_create_client_data(_Ptr<source_t> source, _Ptr<client_t> client)
{
    mp3_client_data *client_mp3 = calloc<mp3_client_data>(1,sizeof(mp3_client_data));
    mp3_state *source_mp3 = source->format->_state;
    const char *metadata;
