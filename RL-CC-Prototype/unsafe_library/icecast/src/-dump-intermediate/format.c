...
static int format_prepare_headers (_Ptr<source_t> source, client_t *client);
...
int format_get_plugin(format_type_t type, _Ptr<struct source_tag> source)
{
    ...
    if (ret < 0)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("content-type"), 
 ...
 */
static void find_client_start (_Ptr<source_t> source, client_t *client)
{
...
static int get_file_data (_Ptr<FILE> intro, client_t *client)
{
...
int format_check_http_buffer (_Ptr<struct source_tag> source, client_t *client : itype(_Ptr<client_t>))
{
    ...
    if (client->respcode == 0)
    {
        ...
        client->respcode = 200;
        stats_event_inc (NULL, _Assume_bounds_cast<_Ptr<const char>>("listeners"));
        stats_event_inc (NULL, _Assume_bounds_cast<_Ptr<const char>>("listener_connections"));
        stats_event_inc (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("listener_connections"));
    }
...
static int format_prepare_headers (_Ptr<source_t> source, client_t *client)
{
    ...
    if (bytes <= 0) {
        ...
        void *new_ptr = realloc<char>(ptr, bytes + 1024);
        if (new_ptr) {
    ...
    if (source->format->create_client_data)
        if (source->format->create_client_data (source, _Assume_bounds_cast<_Ptr<client_t>>(client)) < 0) {
