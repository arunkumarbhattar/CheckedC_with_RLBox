...
*/
void logging_access(_Ptr<client_t> client)
{
    char datebuf[128];
    struct tm thetime;
    time_t now;
    ...
    log_write_direct (accesslog,
            _Assume_bounds_cast<_Ptr<const char>>("%s - %H [%s] \"%H %H %H/%H\" %d %llu \"% H\" \"% H\" %llu"),
            client->con->ip,
            username,
            datebuf,
...
void logging_playlist(_Ptr<const char> mount, _Ptr<const char> metadata, long listeners)
{
    char datebuf[128];
    struct tm thetime;
    time_t now;
    ...
    log_write_direct (playlistlog, _Assume_bounds_cast<_Ptr<const char>>("%s|%s|%ld|%s"),
             datebuf,
             mount,
             listeners,
             metadata);
...
void restart_logging (_Ptr<ice_config_t> config)
{
