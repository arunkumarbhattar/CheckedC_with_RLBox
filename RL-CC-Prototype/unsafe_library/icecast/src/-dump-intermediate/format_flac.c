...
static void flac_codec_free (_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec)
{
    ...
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<const char>>("FLAC_version"), NULL);
...
static _Ptr<refbuf_t> process_flac_page(_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec, ogg_page *page : itype(_Ptr<ogg_page>))
{
...
_Ptr<ogg_codec_t> initial_flac_page(_Ptr<format_plugin_t> plugin, ogg_page *page : itype(_Ptr<ogg_page>))
{
    ...
    do
    {
        ...
        parse += 4;
        stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<char>>("FLAC_version"), "%d.%d",  parse[0], parse[1]);
        codec->process_page = process_flac_page;
        codec->codec_free = flac_codec_free;
        codec->headers = 1;
        codec->name = "FLAC";

        format_ogg_attach_header (_Assume_bounds_cast<_Ptr<ogg_state_t>>(ogg_info), page);
        return codec;
    } while (0);
