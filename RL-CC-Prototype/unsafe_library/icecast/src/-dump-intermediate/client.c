...
int client_create (_Ptr<client_t *> c_ptr, _Ptr<connection_t> con, _Ptr<http_parser_t> parser)
{
    ...
    client_t *client = (client_t *)calloc<client_t>(1, sizeof(client_t));
    int ret = -1;
    ...
    stats_event_args (NULL, _Assume_bounds_cast<_Ptr<char>>("clients"), "%d", global.clients);
    client->con = con;
...
void client_destroy(client_t *client)
{
    ...
    if (client->respcode && client->parser)
        logging_access(_Assume_bounds_cast<_Ptr<client_t>>(client));
    ...
    if (client->free_client_data)
        client->free_client_data (_Assume_bounds_cast<_Ptr<struct _client_tag>>(client));
...
int client_check_source_auth (_Ptr<client_t> client, _Ptr<const char> mount)
{
    ...
    do
    {
        ...
        if (connection_check_pass (client->parser, _Assume_bounds_cast<_Ptr<const char>>(user), _Assume_bounds_cast<_Ptr<const char>>(pass)) > 0)
            ret = 0;
    } while (0);
    config_release_config();
    return ret;
...
void client_send_500(client_t *client : itype(_Ptr<client_t>), const char *message : itype(_Ptr<const char>)) {
    ...
    ret = client_send_bytes(_Assume_bounds_cast<_Ptr<client_t>>(client), header, header_len);

    /* only send message if we have one AND if header could have transmitted completly */
    if (message && ret == header_len)
        client_send_bytes(_Assume_bounds_cast<_Ptr<client_t>>(client), message, strlen(message));
