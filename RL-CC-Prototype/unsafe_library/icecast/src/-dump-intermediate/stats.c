...
{
    _Ptr<volatile stats_event_t> head;
    _Ptr<_Ptr<volatile stats_event_t>> tail;
} event_queue_t;
...
static stats_node_t *_find_node(_Ptr<avl_tree> tree, const char *name : itype(_Ptr<const char>)) : itype(_Ptr<stats_node_t>);
static stats_source_t *_find_source(_Ptr<avl_tree> tree, const char *source : itype(_Ptr<const char>)) : itype(_Ptr<stats_source_t>);
...
static _Ptr<stats_event_t> build_event(const char *source : itype(_Ptr<const char>), const char *name : itype(_Ptr<const char>), const char *value : itype(_Ptr<const char>))
{
...
static char *_get_stats(_Ptr<const char> source, _Ptr<const char> name) : itype(_Ptr<char>)
{
...
** to the avl_tree
*/
static stats_node_t *_find_node(_Ptr<avl_tree> stats_tree, const char *name : itype(_Ptr<const char>)) : itype(_Ptr<stats_node_t>)
{
    stats_node_t *stats;
...
** to the avl_tree
*/
static stats_source_t *_find_source(_Ptr<avl_tree> source_tree, const char *source : itype(_Ptr<const char>)) : itype(_Ptr<stats_source_t>)
{
...
static inline void __format_time(char *buffer : itype(_Ptr<char>), size_t len, const char *format : itype(_Ptr<const char>)) {
...
{
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("server_id"), config->server_id);
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("host"), config->hostname);
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("location"), config->location);
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("admin"), config->admin);
}
...
static void *_stats_thread(void *arg)
{
    ...
    stats_event_time (NULL, _Assume_bounds_cast<_Ptr<const char>>("server_start"));
    stats_event_time_iso8601 (NULL, _Assume_bounds_cast<_Ptr<const char>>("server_start_iso8601"));

    /* global currently active stats */
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("clients"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("connections"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("sources"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("stats"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("listeners"), "0");

    /* global accumulating stats */
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("client_connections"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("source_client_connections"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("source_relay_connections"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("source_total_connections"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("stats_connections"), "0");
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("listener_connections"), "0");
...
static _Ptr<stats_event_t> _make_event_from_node(_Ptr<stats_node_t> node, char *source : itype(_Ptr<char>))
{
...
static xmlNodePtr _dump_stats_to_doc(xmlNodePtr root : itype(_Ptr<xmlNode>), const char *show_mount : itype(_Ptr<const char>), int hidden) : itype(_Ptr<xmlNode>)
{
...
static void _register_listener (_Ptr<event_listener_t> listener)
{
    ...
    while (node) {
        event = _make_event_from_node(_Assume_bounds_cast<_Ptr<stats_node_t>>((stats_node_t *)node->key), NULL);
    ...
    while (node) {
        ...
        while (node2) {
            event = _make_event_from_node(_Assume_bounds_cast<_Ptr<stats_node_t>>((stats_node_t *)node2->key), source->source);
            _add_event_to_queue (event, &listener->queue);
...
void *stats_connection(void *arg)
{
    ...
    thread_mutex_lock(&_stats_mutex);
    _stats_threads++;
    stats_event_args (NULL, _Assume_bounds_cast<_Ptr<char>>("stats"), "%d", _stats_threads);
    thread_mutex_unlock(&_stats_mutex);
    ...
    while (_stats_running) {
        ...
        thread_mutex_unlock (&listener.mutex);
        if (event != NULL) {
            if (_send_event_to_client(event, _Assume_bounds_cast<_Ptr<client_t>>(client)) < 0) {
                _free_event(event);
                break;
            }
            _free_event(event);
...
{
    xmlDocPtr doc;
    _Ptr<char> xslpath = util_get_path_from_normalised_uri (uri);
    const char *mount = httpp_get_query_param (client->parser, "mount");

    doc = stats_get_xml (0, _Assume_bounds_cast<_Ptr<const char>>(mount));
