...
static xsltStylesheetPtr xslt_get_stylesheet(const char *fn : itype(_Ptr<const char>)) : itype(_Ptr<xsltStylesheet>) {
    int i;
    int empty = -1;
    struct stat file;
...
void xslt_transform(xmlDocPtr doc : itype(_Ptr<xmlDoc>), _Ptr<const char> xslfilename, client_t *client : itype(_Ptr<client_t>))
{
    xmlDocPtr    res;
    ...
    if (cur == NULL)
    {
        ...
        client_send_404 (_Assume_bounds_cast<_Ptr<client_t>>(client), _Assume_bounds_cast<_Ptr<const char>>("Could not parse XSLT file"));
        return;
    }
    ...
    if (problem == 0)
    {
        ...
        if (ret == -1) {
            ...
            if ( full_len < (ret + len + 64) ) {
                void *new_data;
                full_len = ret + len + 64;
                new_data = realloc<char>(refbuf->data, full_len);
                if (new_data) {
    ...
    else
    {
        ...
        client_send_404 (_Assume_bounds_cast<_Ptr<client_t>>(client), _Assume_bounds_cast<_Ptr<const char>>("XSLT problem"));
    }
    thread_mutex_unlock (&xsltlock);
    xmlFreeDoc(res);
}
