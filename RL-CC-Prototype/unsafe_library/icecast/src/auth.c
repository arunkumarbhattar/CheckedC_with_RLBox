...
static _Ptr<auth_client> auth_client_setup(const char *mount : itype(_Ptr<const char>), client_t *client : itype(_Ptr<client_t>))
{
...
static void auth_client_free (_Ptr<auth_client> auth_user)
{
    ...
    if (auth_user->client)
    {
        ...
        if (client->respcode)
            ...
            client_send_401 (_Assume_bounds_cast<_Ptr<client_t>>(client));
...
static void auth_new_listener (_Ptr<auth_t> auth, _Ptr<auth_client> auth_user)
{
    ...
    if (is_listener_connected (_Assume_bounds_cast<_Ptr<client_t>>(client)) == 0)
    {
    ...
    while (auth->running)
    {
        ...
        if (auth->head)
        {
            ...
            if (auth_user->process)
                auth_user->process (_Assume_bounds_cast<_Ptr<struct auth_tag>>(auth), auth_user);
            else
 ...
 */
static int add_authenticated_listener (_Ptr<const char> mount, _Ptr<mount_proxy> mountinfo, _Ptr<client_t> client)
{
    int ret = 0;
...
int auth_postprocess_listener (_Ptr<auth_client> auth_user)
{
    ...
    ret = add_authenticated_listener (_Assume_bounds_cast<_Ptr<const char>>(auth_user->mount), mountinfo, _Assume_bounds_cast<_Ptr<client_t>>(client));
    config_release_config();

    if (ret < 0)
        client_send_401 (_Assume_bounds_cast<_Ptr<client_t>>(auth_user->client));
    auth_user->client = NULL;
...
void auth_postprocess_source (_Ptr<auth_client> auth_user)
{
    ...
    if (strcmp (req, "/admin.cgi") == 0 || strncmp ("/admin/metadata", req, 15) == 0)
    {
        ...
        admin_handle_request (_Assume_bounds_cast<_Ptr<client_t>>(client), "/admin/metadata");
    }
    else
    {
        ICECAST_LOG_DEBUG("on mountpoint %s", mount);
        source_startup (client, _Assume_bounds_cast<_Ptr<const char>>(mount), 0);
    }
}
...
void auth_add_listener (_Ptr<const char> mount, _Ptr<client_t> client)
{
    ...
    {
        config_release_config ();
        client_send_403 (client, _Assume_bounds_cast<_Ptr<const char>>("mountpoint unavailable"));
        return;
    }
    if (mountinfo && mountinfo->auth)
    {
        ...
        if (mountinfo->auth->pending_count > 100)
        {
            ...
            client_send_403 (client, _Assume_bounds_cast<_Ptr<const char>>("busy, please try again later"));
            return;
        }
    ...
    else
    {
        ...
        config_release_config ();
        if (ret < 0)
            client_send_403 (client, _Assume_bounds_cast<_Ptr<const char>>("max listeners reached"));
    }
}
...
int auth_release_listener (client_t *client : itype(_Ptr<client_t>))
{
    if (client->authenticated)
    {
        _Ptr<const char> mount = httpp_getvar (client->parser, HTTPP_VAR_URI);
...
{
    auth_t *auth = calloc<auth_t> (1, sizeof (auth_t));
...
_Ptr<_Ptr<config_options_t>> next_option = &options;
    ...
    if (get_authenticator (_Assume_bounds_cast<_Ptr<auth_t>>(auth), options) < 0)
    {
        xmlFree (auth->type);
        free<auth_t> (auth);
        auth = NULL;
