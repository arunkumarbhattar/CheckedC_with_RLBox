...
static int connection_send (_Ptr<connection_t> con, const void *buf, size_t len)
{
...
static int accept_ip_address (char *ip : itype(_Ptr<char>))
{
    void *result;
 ...
 */
void connection_uses_ssl (_Ptr<connection_t> con)
{
#ifdef HAVE_OPENSSL
    ...
    while (*node_ref)
    {
        ...
        if (len > 0)
        {
            if (client->con->con_time + timeout <= time(NULL))
                ...
                len = client_read_bytes (_Assume_bounds_cast<_Ptr<client_t>>(client), buf, len);
        }
...
void connection_accept_loop (void)
{
    ...
    while (global.running == ICECAST_RUNNING)
    {
        ...
        if (con)
        {
            ...
            {
                global_unlock();
                client_send_403 (_Assume_bounds_cast<_Ptr<client_t>>(client), _Assume_bounds_cast<_Ptr<const char>>("Icecast connection limit reached"));
            ...
            stats_event_inc (NULL, _Assume_bounds_cast<_Ptr<const char>>("connections"));
            duration = 5;
        }
        else
        {
...
int connection_complete_source (_Ptr<struct source_tag> source, int response)
{
    ...
    {
        _Ptr<const char> contenttype = ((void *)0);
        _Ptr<const char> expectcontinue = ((void *)0);
        ...
        if (contenttype != NULL)
        {
            ...
            if (format_type == FORMAT_ERROR)
            {
                ...
                global_unlock();
                if (response) {
                    client_send_403 (_Assume_bounds_cast<_Ptr<client_t>>(source->client), _Assume_bounds_cast<_Ptr<const char>>("Content-type not supported"));
                    source->client = NULL;
                }
            ...
            global_unlock();
            if (response) {
                client_send_403 (_Assume_bounds_cast<_Ptr<client_t>>(source->client), _Assume_bounds_cast<_Ptr<const char>>("No Content-type given"));
                source->client = NULL;
            }
        ...
        if (format_get_plugin (format_type, source) < 0)
        {
            ...
            if (response)
            {
                client_send_403 (_Assume_bounds_cast<_Ptr<client_t>>(source->client), _Assume_bounds_cast<_Ptr<const char>>("internal format allocation problem"));
                source->client = NULL;
            }
...
#endif
	    {
		client_send_100 (_Assume_bounds_cast<_Ptr<client_t>>(source->client));
	    }
	}
    ...
    if (response)
    {
        client_send_403 (_Assume_bounds_cast<_Ptr<client_t>>(source->client), _Assume_bounds_cast<_Ptr<const char>>("too many sources connected"));
        source->client = NULL;
    }
...
static int _check_pass_http(_Ptr<http_parser_t> parser, const char *correctuser : itype(_Ptr<const char>), const char *correctpass : itype(_Ptr<const char>))
{
...
static int _check_pass_icy(_Ptr<http_parser_t> parser, const char *correctpass : itype(_Ptr<const char>))
{
    const char *password;
...
static int _check_pass_ice(_Ptr<http_parser_t> parser, const char *correctpass : itype(_Ptr<const char>))
{
    const char *password;
...
static void _handle_source_request (_Ptr<client_t> client, const char *uri : itype(_Ptr<const char>))
{
    ...
    switch (client_check_source_auth (client, _Assume_bounds_cast<_Ptr<const char>>(uri)))
    {
        case 0: /* authenticated from config file */
            source_startup (client, _Assume_bounds_cast<_Ptr<const char>>(uri), ICECAST_SOURCE_AUTH);
            break;
...
void source_startup (client_t *client : itype(_Ptr<client_t>), _Ptr<const char> uri, int auth_style)
{
    ...
    {
        client_send_403 (_Assume_bounds_cast<_Ptr<client_t>>(client), _Assume_bounds_cast<_Ptr<const char>>("Mountpoint in use"));
...
{
    stats_event_inc(NULL, _Assume_bounds_cast<_Ptr<const char>>("stats_connections"));
...
static void _handle_get_request (_Ptr<client_t> client, char *passed_uri : itype(_Ptr<char>))
{
    ...
    */
    ...
    stats_event_inc(NULL, _Assume_bounds_cast<_Ptr<const char>>("client_connections"));
    ...
    auth_add_listener (_Assume_bounds_cast<_Ptr<const char>>(uri), client);
    if (uri != passed_uri) free<char> (uri);
}
...
static void _handle_shoutcast_compatible (_Ptr<client_queue_t> node)
{
    ...
    if (node->shoutcast == 1)
    {
        ...
        *ptr = '\0';
        ...
        if (source_password && strcmp (client->refbuf->data, source_password) == 0)
        {
            ...
            client_send_bytes(_Assume_bounds_cast<_Ptr<client_t>>(client), "OK2\r\nicy-caps:11\r\n\r\n", 20); /* TODO: Replace Magic Number! */
    ...
    if (httpp_parse (parser, http_compliant, strlen(http_compliant)))
    {
        ...
        source_startup (client, _Assume_bounds_cast<_Ptr<const char>>(shoutcast_mount), SHOUTCAST_SOURCE_AUTH);
    }
    else {
        ...
        if (node)
        {
            ...
            if (httpp_parse (parser, client->refbuf->data, node->offset))
            {
                ...
                if (node->shoutcast_mount && strcmp (rawuri, "/admin.cgi") == 0)
                    httpp_set_query_param (client->parser, "mount", _Assume_bounds_cast<_Ptr<const char>>(node->shoutcast_mount));
                ...
                if (parser->req_type == httpp_req_source || parser->req_type == httpp_req_put) {
                    _handle_source_request (_Assume_bounds_cast<_Ptr<client_t>>(client), uri);
                }
                else if (parser->req_type == httpp_req_stats) {
                    _handle_stats_request (_Assume_bounds_cast<_Ptr<client_t>>(client), uri);
                }
                else if (parser->req_type == httpp_req_get) {
                    _handle_get_request (_Assume_bounds_cast<_Ptr<client_t>>(client), uri);
                }
                else {
                    ICECAST_LOG_ERROR("Wrong request type from client");
                    client_send_400 (_Assume_bounds_cast<_Ptr<client_t>>(client), "unknown request");
                }
