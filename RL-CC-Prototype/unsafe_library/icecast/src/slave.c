...
static _Ptr<thread_type> _slave_thread_id = ((void *)0);
...
_Ptr<relay_server> relay_free(_Ptr<relay_server> relay)
{
...
_Ptr<relay_server> relay_copy(_Ptr<relay_server> r)
{
 ...
 */
static client_t *open_relay_connection(_Ptr<relay_server> relay) : itype(_Ptr<client_t>)
{
    int redirects = 0;
    char *server_id = NULL;
    ...
    while (redirects < 10)
    {
         ...
         */
        sock_write(streamsock, _Assume_bounds_cast<_Ptr<const char>>("GET %s HTTP/1.0\r\n"
                "User-Agent: %s\r\n"
                "Host: %s\r\n"
                "%s"
                "%s"
        ...
        if (! httpp_parse_response (parser, header, strlen(header), _Assume_bounds_cast<_Ptr<const char>>(relay->localmount)))
        {
...
static void *start_relay_stream (void *arg)
{
    ...
    do
    {
        client = open_relay_connection (_Assume_bounds_cast<_Ptr<relay_server>>(relay));
        ...
        stats_event_inc(NULL, _Assume_bounds_cast<_Ptr<const char>>("source_relay_connections"));
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(relay->localmount), _Assume_bounds_cast<_Ptr<const char>>("source_ip"), client->con->ip);
    ...
    if (relay->source == NULL)
    {
        ...
        if (relay->source)
        {
            ...
            if (relay->on_demand)
            {
                ...
                config_release_config ();
                stats_event (_Assume_bounds_cast<_Ptr<const char>>(relay->localmount), _Assume_bounds_cast<_Ptr<const char>>("listeners"), "0");
                slave_update_all_mounts();
            }
        }
        else
...
static _Ptr<relay_server> update_relay_set(_Ptr<_Ptr<relay_server>> current, _Ptr<relay_server> updated)
{
    ...
    _Ptr<relay_server> existing_relay = ((void *)0);
_Ptr<_Ptr<relay_server>> existing_p = ((void *)0);
...
static void relay_check_streams (_Ptr<relay_server> to_start, _Ptr<relay_server> to_free, int skip_timer)
{
    ...
    while (to_free)
    {
        if (to_free->source)
        {
            if (to_free->running)
            {
                ...
                stats_event (_Assume_bounds_cast<_Ptr<const char>>(to_free->localmount), NULL, NULL);
        }
...
_Ptr<relay_server> cleanup_relays = ((void *)0);
        ...
        ret = 1;
        config_release_config();
        mastersock = sock_connect_wto (_Assume_bounds_cast<_Ptr<const char>>(master), port, 10);
        ...
        authheader = malloc<char>(len);
        snprintf (authheader, len, "%s:%s", username, password);
        data = util_base64_encode(authheader);
        sock_write (mastersock,
                _Assume_bounds_cast<_Ptr<const char>>("GET /admin/streamlist.txt HTTP/1.0\r\n"
                "Authorization: Basic %s\r\n"
                "\r\n"), data);
