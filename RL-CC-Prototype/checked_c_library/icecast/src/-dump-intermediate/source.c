...
static void _parse_audio_info (_Ptr<source_t> source, const char *s : itype(_Ptr<const char>));
...
#else
static void source_run_script (char *command : itype(_Ptr<char>), _Ptr<char> mountpoint);
#endif
...
void source_clear_source (_Ptr<source_t> source)
{
    ...
    if (c)
    {
        stats_event_sub (NULL, _Assume_bounds_cast<_Ptr<const char>>("listeners"), source->listeners);
...
void source_move_clients (_Ptr<source_t> source, _Ptr<source_t> dest)
{
    ...
    do
    {
        ...
        source->listeners = 0;
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("listeners"), "0");
...
static _Ptr<refbuf_t> get_next_buffer(_Ptr<source_t> source)
{
    ...
    while (global.running == ICECAST_RUNNING && source->running)
    {
        ...
        {
            stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("total_bytes_read"),
                    "%"PRIu64, source->format->read_bytes);
            stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("total_bytes_sent"),
...
static void send_to_listener (_Ptr<source_t> source, _Ptr<client_t> client, int deletion_expected)
{
    ...
    if (deletion_expected && client->refbuf && client->refbuf == source->stream_data)
    {
        ...
        stats_event_inc (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("slow_listeners"));
        client->con->error = 1;
    }
}
 ...
 */
static _Ptr<FILE> source_open_dumpfile(const char *filename : itype(_Ptr<const char>)) {
#ifndef _WIN32
...
static void source_init (_Ptr<source_t> source)
{
    ...
    if (str)
    {
        ...
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("audio_info"), str);
    }

    stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("listenurl"), listenurl);
    ...
    source->listeners = 0;
    stats_event_inc (NULL, _Assume_bounds_cast<_Ptr<const char>>("source_total_connections"));
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("slow_listeners"), "0");
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("listeners"), "%lu", source->listeners);
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("listener_peak"), "%lu", source->peak_listeners);
    stats_event_time (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("stream_start"));
    stats_event_time_iso8601 (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("stream_start_iso8601"));
    ...
    while (global.running == ICECAST_RUNNING && source->running) {
        ...
        while (client_node) {
            ...
            if (client->con->error) {
                ...
                if (client->respcode == 200)
                    stats_event_dec (NULL, _Assume_bounds_cast<_Ptr<const char>>("listeners"));
        ...
        while (client_node) {
            ...
            stats_event_inc(_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("connections"));
...
static void source_shutdown (_Ptr<source_t> source)
{
    ...
    stats_event(_Assume_bounds_cast<_Ptr<const char>>(source->mount), NULL, NULL);
    ...
    global_lock();
    global.sources--;
    stats_event_args (NULL, _Assume_bounds_cast<_Ptr<char>>("sources"), "%d", global.sources);
    global_unlock();
...
static int _free_client(void *key)
{
    ...
    if (client->respcode == 0)
        client_send_404 (_Assume_bounds_cast<_Ptr<client_t>>(client), _Assume_bounds_cast<_Ptr<const char>>("Mount unavailable"));
    else
    ...
    while (start != NULL && *start != '\0')
    {
        ...
        if (len)
        {
            ...
            if (esc)
            {
                ...
                stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>(name), esc);
                free<char> (esc);
            }
        }
        start = s;
...
static void source_apply_mount (_Ptr<source_t> source, _Ptr<mount_proxy> mountinfo)
{
    ...
    if (source->format && source->format->apply_settings)
        source->format->apply_settings (_Assume_bounds_cast<_Ptr<client_t>>(source->client), source->format, mountinfo);
    ...
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("public"), "%d", val);
    ...
    if (mountinfo && mountinfo->stream_name)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_name"), mountinfo->stream_name);
    else
    {
        do {
        ...
        if (source->format)
            stats_event_conv (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_name"), str, source->format->charset);
    }

    /* stream description */
    if (mountinfo && mountinfo->stream_description)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_description"), mountinfo->stream_description);
    else
    {
        do {
        ...
        if (source->format)
            stats_event_conv (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_description"), str, source->format->charset);
    }

    /* stream URL */
    if (mountinfo && mountinfo->stream_url)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_url"), mountinfo->stream_url);
    else
    {
        do {
        ...
        if (str && source->format)
            stats_event_conv (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_url"), str, source->format->charset);
    }

    /* stream genre */
    if (mountinfo && mountinfo->stream_genre)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("genre"), mountinfo->stream_genre);
    else
    {
        do {
        ...
        if (source->format)
            stats_event_conv (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("genre"), str, source->format->charset);
    }
    ...
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("bitrate"), str);

    /* handle MIME-type */
    if (mountinfo && mountinfo->type)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_type"), mountinfo->type);
    else
        if (source->format)
            stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("server_type"), source->format->contenttype);

    if (mountinfo && mountinfo->subtype)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("subtype"), mountinfo->subtype);

    if (mountinfo && mountinfo->auth)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("authenticator"), mountinfo->auth->type);
    else
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("authenticator"), NULL);
...
void source_update_settings (_Ptr<ice_config_t> config, _Ptr<source_t> source, _Ptr<mount_proxy> mountinfo)
{
    ...
    {
        stats_event_hidden (_Assume_bounds_cast<_Ptr<const char>>(source->mount), NULL, 1);
        thread_mutex_unlock(&source->lock);
        return;
    }
    ...
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("listenurl"), "http://%s:%d%s",
            config->hostname, config->port, source->mount);
    ...
    if (source->on_demand)
    {
        ...
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("on_demand"), "1");
        stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<char>>("listeners"), "%ld", source->listeners);
    }
    else
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("on_demand"), NULL);
    ...
    if (source->hidden)
    {
        ...
        stats_event_hidden (_Assume_bounds_cast<_Ptr<const char>>(source->mount), NULL, 0);

    if (source->max_listeners == -1)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("max_listeners"), "unlimited");
    else
    {
        char buf [10];
        snprintf (buf, sizeof (buf), "%ld", source->max_listeners);
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("max_listeners"), buf);
    }
...
void *source_client_thread (void *arg)
{
    ...
    stats_event_inc(NULL, _Assume_bounds_cast<_Ptr<const char>>("source_client_connections"));
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("listeners"), "0");
...
void source_client_callback (client_t *client : itype(_Ptr<client_t>), void *arg)
{
    ...
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("source_ip"), source->client->con->ip);
    agent = httpp_getvar (source->client->parser, "user-agent");
    if (agent)
        stats_event (_Assume_bounds_cast<_Ptr<const char>>(source->mount), _Assume_bounds_cast<_Ptr<const char>>("user_agent"), agent);
...
static void *source_fallback_file (void *arg)
{
    ...
    do
    {
        ...
        type = fserve_content_type (_Assume_bounds_cast<_Ptr<const char>>(mount));
...
void source_recheck_mounts (int update_all)
{
    ...
    for (; mount; mount = mount->next)
    {
        ...
        if (source)
        {
            ...
            {
                stats_event_hidden (_Assume_bounds_cast<_Ptr<const char>>(mount->mountname), NULL, mount->hidden);
                stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(mount->mountname), _Assume_bounds_cast<_Ptr<char>>("listenurl"), "http://%s:%d%s",
                        config->hostname, config->port, mount->mountname);
                stats_event (_Assume_bounds_cast<_Ptr<const char>>(mount->mountname), _Assume_bounds_cast<_Ptr<const char>>("listeners"), "0");
                if (mount->max_listeners < 0)
                    stats_event (_Assume_bounds_cast<_Ptr<const char>>(mount->mountname), _Assume_bounds_cast<_Ptr<const char>>("max_listeners"), "unlimited");
                else
                    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(mount->mountname), _Assume_bounds_cast<_Ptr<char>>("max_listeners"), "%d", mount->max_listeners);
            }
        }
        else
            stats_event (_Assume_bounds_cast<_Ptr<const char>>(mount->mountname), NULL, NULL);
