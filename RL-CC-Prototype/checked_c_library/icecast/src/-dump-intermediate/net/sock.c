...
*/
int sock_write_string(sock_t sock, const char *buff : itype(_Ptr<const char>))
{
...
*/
int sock_write(sock_t sock, _Ptr<const char> fmt, ...)
{
    int rc;
    va_list ap;
...
*/
int sock_read_line(sock_t sock, char *buff : itype(_Ptr<char>), const int len)
{
    char c = '\0';
    int read_bytes, pos;
...
sock_t sock_connect_wto (_Ptr<const char> hostname, int port, int timeout)
{
...
sock_t sock_connect_non_blocking (const char *hostname : itype(_Ptr<const char>), unsigned port)
{
    int sock = SOCK_ERROR;
...
sock_t sock_get_server_socket (int port, const char *sinterface : itype(_Ptr<const char>))
{
...
sock_t sock_accept(sock_t serversock, char *ip : itype(_Ptr<char>), size_t len)
{
