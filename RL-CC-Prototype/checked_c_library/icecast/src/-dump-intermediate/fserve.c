...
void fserve_initialize(void)
{
    ...
    stats_event (NULL, _Assume_bounds_cast<_Ptr<const char>>("file_connections"), "0");
...
static int wait_for_fds(void)
{
    ...
    while (run_fserv)
    {
        ...
        if (pending_list)
        {
            ...
            fclient = (_Ptr<fserve_t>)pending_list;
            while (fclient)
            {
...
{
    _Ptr<fserve_t> fclient = ((void *)0);
_Ptr<_Ptr<fserve_t>> trail = ((void *)0);
    size_t bytes;
    ...
    while (1)
    {
        ...
        while (fclient)
        {
            ...
            if (fclient->ready)
            {
                ...
                format_generic_write_to_client (_Assume_bounds_cast<_Ptr<client_t>>(client));
...
int fserve_client_create (_Ptr<client_t> httpclient, const char *path : itype(_Ptr<const char>))
{
    ...
    if (stat (fullpath, &file_buf) != 0)
    {
        ...
        if (m3u_requested == 0 && xslt_playlist_requested == NULL)
        {
            ...
            client_send_404 (httpclient, _Assume_bounds_cast<_Ptr<const char>>("The file you requested could not be found"));
            free<char> (fullpath);
            return -1;
        }
    ...
    if (xslt_playlist_requested && xslt_playlist_file_available == 0)
    {
        ...
        if (eol)
            *eol = '\0';
        doc = stats_get_xml (0, _Assume_bounds_cast<_Ptr<const char>>(reference));
        free<char> (reference);
    ...
    if (file == NULL)
    {
        ...
        client_send_404 (httpclient, _Assume_bounds_cast<_Ptr<const char>>("File not readable"));
        free<char> (fullpath);
        return -1;
    }
    free<char> (fullpath);
    ...
    if (range != NULL) {
        ...
        if (!rangeproblem) {
            ...
            if (!rangeproblem) {
                ...
                type = fserve_content_type (_Assume_bounds_cast<_Ptr<const char>>(path));
        ...
        _Ptr<char> type = fserve_content_type(_Assume_bounds_cast<_Ptr<const char>>(path));
        httpclient->respcode = 200;
	bytes = util_http_build_header (httpclient->refbuf->data, BUFSIZE, 0,
	                                0, 200, NULL,
					type, NULL,
    ...
    stats_event_inc (NULL, _Assume_bounds_cast<_Ptr<const char>>("file_connections"));
    fserve_add_client (httpclient, _Assume_bounds_cast<_Ptr<FILE>>(file));
...
int fserve_add_client (client_t *client : itype(_Ptr<client_t>), _Ptr<FILE> file)
{
    ...
    if (fclient == NULL)
    {
        client_send_404 (_Assume_bounds_cast<_Ptr<client_t>>(client), _Assume_bounds_cast<_Ptr<const char>>("memory exhausted"));
        return -1;
    }
    fclient->file = file;
