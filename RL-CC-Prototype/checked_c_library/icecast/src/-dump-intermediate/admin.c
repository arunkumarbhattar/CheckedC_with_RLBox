...
int admin_get_command(const char *command : itype(_Ptr<const char>))
{
...
static void command_stats(_Ptr<client_t> client, _Ptr<const char> mount, int response);
...
static void command_buildm3u(_Ptr<client_t> client, const char *mount : itype(_Ptr<const char>));
...
xmlDocPtr admin_build_sourcelist(const char *mount : itype(_Ptr<const char>)) : itype(_Ptr<xmlDoc>)
{
...
void admin_send_response (xmlDocPtr doc : itype(_Ptr<xmlDoc>), client_t *client : itype(_Ptr<client_t>), int response, const char *xslt_template)
{
    if (response == RAW)
    {
        xmlChar *buff = NULL;
	...
	ret = util_http_build_header(client->refbuf->data, buf_len, 0,
        ...
        if (ret == -1) {
            ...
            void *new_data;
            buf_len = ret + len + 64;
            new_data = realloc<char>(client->refbuf->data, buf_len);
            if (new_data) {
    ...
    if (response == TRANSFORMED)
    {
        ...
        xslt_transform(doc, _Assume_bounds_cast<_Ptr<const char>>(fullpath_xslt_template), client);
        free<char>(fullpath_xslt_template);
    }
}


void admin_handle_request(_Ptr<client_t> client, const char *uri : itype(_Ptr<const char>))
{
    ...
    if (command == COMMAND_SHOUTCAST_METADATA_UPDATE) {
        ...
        httpp_set_query_param (client->parser, "mount", _Assume_bounds_cast<_Ptr<const char>>(sc_mount));
    ...
    if(mount != NULL) {
        ...
        {
            switch (client_check_source_auth (client, _Assume_bounds_cast<_Ptr<const char>>(mount)))
            {
                case 0:
                    break;
                default:
...
static void admin_handle_mount_request(_Ptr<client_t> client, _Ptr<source_t> source, int command)
{
    switch(command) {
        case COMMAND_RAW_STATS:
            command_stats(client, _Assume_bounds_cast<_Ptr<const char>>(source->mount), RAW);
            break;
        ...
        case COMMAND_RAW_KILL_SOURCE:
            ...
            break;
        case COMMAND_TRANSFORMED_STATS:
            command_stats(client, _Assume_bounds_cast<_Ptr<const char>>(source->mount), TRANSFORMED);
            break;
...
static void html_success(_Ptr<client_t> client, char *message : itype(_Ptr<char>))
{
    ssize_t ret;
...
static void command_metadata(_Ptr<client_t> client, _Ptr<source_t> source, int response)
{
    ...
    if (same_ip && plugin && plugin->set_tag)
    {
        if (song)
        {
            plugin->set_tag (plugin, "song", song, _Assume_bounds_cast<_Ptr<const char>>(charset));
        ...
        else
        {
            if (artist && title)
            {
                plugin->set_tag (plugin, "title", title, _Assume_bounds_cast<_Ptr<const char>>(charset));
                plugin->set_tag (plugin, "artist", artist, _Assume_bounds_cast<_Ptr<const char>>(charset));
