...
static auth_result htpasswd_adduser (_Ptr<auth_t> auth, const char *username : itype(_Ptr<const char>), const char *password : itype(_Ptr<const char>));
static auth_result htpasswd_deleteuser(_Ptr<auth_t> auth, const char *username : itype(_Ptr<const char>));
static auth_result htpasswd_userlist(_Ptr<auth_t> auth, xmlNodePtr srcnode : itype(_Ptr<xmlNode>));
...
/* md5 hash */
static _Ptr<char> get_hash(const char *data, int len)
{
...
static void htpasswd_recheckfile (_Ptr<htpasswd_auth_state> htpasswd)
{
    ...
    while (get_line(passwdfile, line, MAX_LINE_LEN))
    {
        ...
        *sep = 0;
        memcpy<char> (entry->name, line, len);
...
static auth_result htpasswd_auth (_Ptr<auth_client> auth_user)
{
    ...
    if (avl_get_by_key (htpasswd->users, &entry, &result) == 0)
    {
        ...
        thread_rwlock_unlock (&htpasswd->file_rwlock);
        hashed_pw = ((char *)get_hash (client->password, strlen (client->password)));
...
static auth_result htpasswd_adduser (_Ptr<auth_t> auth, const char *username : itype(_Ptr<const char>), const char *password : itype(_Ptr<const char>))
{
    ...
    hashed_password = ((char *)get_hash(password, strlen(password)));
    if (hashed_password) {
