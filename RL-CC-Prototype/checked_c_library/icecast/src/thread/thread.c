...
static _Ptr<avl_tree> _threadtree = NULL;
...
void thread_initialize(void)
{
    ...
    */
    ...
    thread = (thread_type *)malloc<thread_type>(sizeof(thread_type));
...
thread_type *thread_create_c(char *name, _Ptr<void *(void *)> start_routine, void *arg, int detached, int line, char *file) : itype(_Ptr<thread_type>)
{
    int ok = 1;
    ...
    thread = (thread_type *)calloc<thread_type>(1, sizeof(thread_type));    
    do {
        if (thread == NULL)
            break;
        start = (thread_start_t *)calloc<thread_start_t>(1, sizeof(thread_start_t));
        if (start == NULL)
            break;
...
void thread_mutex_create_c(_Ptr<mutex_t> mutex, int line, char *file)
{
    _mutex_create(mutex);
...
void thread_mutex_lock_c(_Ptr<mutex_t> mutex, int line, char *file)
{
#ifdef DEBUG_MUTEXES
...
void thread_mutex_unlock_c(_Ptr<mutex_t> mutex, int line, char *file)
{
#ifdef DEBUG_MUTEXES
    thread_type *th = thread_self();
...
void thread_cond_create_c(_Ptr<cond_t> cond, int line, char *file)
{
...
void thread_cond_broadcast_c(_Ptr<cond_t> cond, int line, char *file)
{
...
void thread_rwlock_create_c(_Ptr<rwlock_t> rwlock, int line, char *file)
{
...
void thread_rwlock_rlock_c(_Ptr<rwlock_t> rwlock, int line, char *file)
{
    pthread_rwlock_rdlock(&rwlock->sys_rwlock);
}

void thread_rwlock_wlock_c(_Ptr<rwlock_t> rwlock, int line, char *file)
{
    pthread_rwlock_wrlock(&rwlock->sys_rwlock);
}

void thread_rwlock_unlock_c(_Ptr<rwlock_t> rwlock, int line, char *file)
{
...
void thread_rename(const char *name : itype(_Ptr<const char>))
{
    _Ptr<thread_type> th = ((void *)0);
