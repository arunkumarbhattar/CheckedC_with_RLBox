...
typedef struct vorbis_codec_tag
{
    ...
    _Ptr<int (_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec)> process_packet;
    _Ptr<_Ptr<refbuf_t> (_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec)> get_buffer_page;

} vorbis_codec_t;

static int process_vorbis_headers (_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec);
static _Ptr<refbuf_t> process_vorbis_page(_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec, ogg_page *page : itype(_Ptr<ogg_page>));
static _Ptr<refbuf_t> process_vorbis(_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec);
static void vorbis_set_tag (_Ptr<format_plugin_t> plugin, const char *tag : itype(_Ptr<const char>), const char *value : itype(_Ptr<const char>), _Ptr<const char> charset);
...
static void vorbis_codec_free (_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec)
{
    ...
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<const char>>("audio_bitrate"), NULL);
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<const char>>("audio_channels"), NULL);
    stats_event (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<const char>>("audio_samplerate"), NULL);
...
static ogg_packet *copy_ogg_packet(ogg_packet *packet : itype(_Ptr<ogg_packet>)) : itype(_Ptr<ogg_packet>)
{
    ogg_packet *next;
    do
    {
 ...
 */
_Ptr<ogg_codec_t> initial_vorbis_page(_Ptr<format_plugin_t> plugin, ogg_page *page : itype(_Ptr<ogg_page>))
{
    _Ptr<ogg_codec_t> codec = calloc<ogg_codec_t> (1, sizeof (ogg_codec_t));
    ogg_packet packet;

    vorbis_codec_t *vorbis = calloc<vorbis_codec_t> (1, sizeof (vorbis_codec_t));
 ...
 */
static void vorbis_set_tag (_Ptr<format_plugin_t> plugin, const char *tag : itype(_Ptr<const char>), const char *in_value : itype(_Ptr<const char>), _Ptr<const char> charset)
{   
 ...
 */
static _Ptr<refbuf_t> process_vorbis_passthru_page(_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec, ogg_page *page : itype(_Ptr<ogg_page>))
{
...
static _Ptr<refbuf_t> process_vorbis_page(_Ptr<ogg_state_t> ogg_info, _Ptr<ogg_codec_t> codec, ogg_page *page : itype(_Ptr<ogg_page>))
{
    ...
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<char>>("audio_samplerate"), "%ld", (long)source_vorbis->vi.rate);
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<char>>("audio_channels"), "%ld", (long)source_vorbis->vi.channels);
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<char>>("audio_bitrate"), "%ld", (long)source_vorbis->vi.bitrate_nominal);
    stats_event_args (_Assume_bounds_cast<_Ptr<const char>>(ogg_info->mount), _Assume_bounds_cast<_Ptr<char>>("ice-bitrate"), "%ld", (long)source_vorbis->vi.bitrate_nominal/1000);
