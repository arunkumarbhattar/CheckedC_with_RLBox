...
_Ptr<avl_node> avl_node_new(void *        key, _Ptr<avl_node> parent)
{
...
_Ptr<avl_tree> avl_tree_new(avl_key_compare_fun_type compare_fun, void * compare_arg)
{
  _Ptr<avl_tree> t = (_Ptr<avl_tree>) malloc<avl_tree> (sizeof (avl_tree));
...
static void
avl_tree_free_helper (_Ptr<avl_node> node, avl_free_key_fun_type free_key_fun)
{
  if (node->left) {
...
void
avl_tree_free (_Ptr<avl_tree> tree, avl_free_key_fun_type free_key_fun)
{
  if (tree->length) {
...
int
avl_insert (_Ptr<avl_tree> ob, void * key)
{
  if (!(ob->root->right)) {
    ...
    _Ptr<avl_node> t = ((void *)0);
_Ptr<avl_node> p = ((void *)0);
_Ptr<avl_node> s = ((void *)0);
_Ptr<avl_node> q = ((void *)0);
_Ptr<avl_node> r = ((void *)0);
    int a;
...
int
avl_get_by_index (_Ptr<avl_tree> tree, unsigned long index, void ** value_address)
{
...
int
avl_get_by_key (_Ptr<avl_tree> tree, void * key, void **value_address)
{
...
int avl_delete(_Ptr<avl_tree> tree, void *key, avl_free_key_fun_type free_key_fun)
{
  _Ptr<avl_node> x = ((void *)0);
_Ptr<avl_node> y = ((void *)0);
_Ptr<avl_node> p = ((void *)0);
_Ptr<avl_node> q = ((void *)0);
_Ptr<avl_node> r = ((void *)0);
_Ptr<avl_node> top = ((void *)0);
_Ptr<avl_node> x_child = ((void *)0);
...
static int
avl_iterate_inorder_helper (_Ptr<avl_node> node, avl_iter_fun_type iter_fun, void * iter_arg)
{
  int result;
  if (node->left) {
...
int
avl_iterate_inorder (_Ptr<avl_tree> tree, avl_iter_fun_type iter_fun, void * iter_arg)
{
  int result;
...
_Ptr<avl_node> avl_get_first(_Ptr<avl_tree> tree)
{
...
_Ptr<avl_node> avl_get_prev(_Ptr<avl_node> node)
{
    if (node->left) {
        node = node->left;
        while (node->right) {
...
_Ptr<avl_node> avl_get_next(_Ptr<avl_node> node)
{
    if (node->right) {
        node = node->right;
        while (node->left) {
...
int
avl_iterate_index_range (_Ptr<avl_tree> tree, avl_iter_index_fun_type iter_fun, unsigned long low, unsigned long high, void * iter_arg)
{
  unsigned long m;
...
static _Ptr<avl_node> avl_get_index_by_key(_Ptr<avl_tree> tree, void * key, _Ptr<unsigned long> index)
{
  _Ptr<avl_node> x = tree->root->right;
  unsigned long m;
...
int
avl_get_span_by_key (_Ptr<avl_tree> tree, void * key, _Ptr<unsigned long> low, _Ptr<unsigned long> high)
{
  unsigned long m, i, j;
  _Ptr<avl_node> node = ((void *)0);
...
int
avl_get_span_by_two_keys (_Ptr<avl_tree> tree, void * low_key, void * high_key, _Ptr<unsigned long> low, _Ptr<unsigned long> high)
{
  unsigned long i, j;
...
int
avl_get_item_by_key_most (_Ptr<avl_tree> tree, void * key, void **value_address)
{
  _Ptr<avl_node> x = tree->root->right;
  *value_address = NULL;
...
int
avl_get_item_by_key_least (_Ptr<avl_tree> tree, void * key, void **value_address)
{
  _Ptr<avl_node> x = tree->root->right;
  *value_address = NULL;
...
static int
default_key_printer (char *buffer : itype(_Ptr<char>), void * key)
{
...
void
avl_print_tree (_Ptr<avl_tree> tree, avl_key_printer_fun_type key_printer)
{
...
void avl_tree_rlock(_Ptr<avl_tree> tree)
{
    thread_rwlock_rlock(&tree->rwlock);
}

void avl_tree_wlock(_Ptr<avl_tree> tree)
{
    thread_rwlock_wlock(&tree->rwlock);
}

void avl_tree_unlock(_Ptr<avl_tree> tree)
{
