...
static int create_log_entry (int log_id, const char *pre : itype(_Ptr<const char>), const char *line : itype(_Ptr<const char>))
{
    ...
    entry->line = malloc<char> (entry->len);
...
static void __vsnprintf(char *str : itype(_Ptr<char>), size_t size, const char *format : itype(_Ptr<const char>), va_list ap) {
    int in_block = 0;
    int block_size = 0;
    int block_len;
    int block_space = 0;
